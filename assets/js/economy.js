...
export function setupEconomy() {
    ...
    const modifyCoins = (isAdding) => {
        ...
        let changeKnuts = 0;
        if(type === 'g') changeKnuts = amount * 493;
        if(type === 's') changeKnuts = amount * 29;
        if(type === 'k') changeKnuts = amount;

        let totalKnuts = toKnuts(current.g, current.s, current.k);
        totalKnuts = isAdding ? totalKnuts + changeKnuts : totalKnuts - changeKnuts;

        const std = toStandard(totalKnuts);
        setCoins(std.g, std.s, std.k);
